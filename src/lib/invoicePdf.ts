/**
 * Invoice PDF Generator - OnSite Timekeeper
 *
 * Generates professional invoice PDFs for both hourly and products/services invoices.
 * Reuses expo-print infrastructure from timesheetPdf.ts.
 */
...
<body>
  ...
  <div class="footer">
    this document was generated by onsite timekeeper &middot; onsiteclub.ca
  </div>
</body>
</html>
  `.trim();
...
/**
 * Generate invoice PDF file from HTML.
 * Returns the file URI for caching in the invoices table.
 * Includes a 30s timeout to prevent indefinite hangs from the native print renderer.
 *
 * If `xmpMetadata` is supplied, the onsite:* XMP metadata packet is embedded
 * into the PDF after generation. This is invisible to humans reading the PDF
 * but lets downstream systems (OnSite Ops) parse invoice fields when the
